# Whyline — Complete Project Specification

> This is the single source of truth for building Whyline.
...
Whyline is a CLI tool that captures engineering decisions through natural conversation and makes them retrievable through natural language queries. The user talks, the system structures, stores, and indexes. No templates, no forms, no manual organization.
...
**Rebuild guarantee:** `graph/` and `vectors/` are fully disposable. `whyline rebuild` reconstructs both entirely from the markdown files. This is the recovery mechanism, migration path, and proof that the filesystem is the true source of truth.
...
Swappable via config. Changing model requires `whyline rebuild` to re-embed everything.
...
- Future: `whyline merge <node-a> <node-b>` to combine nodes (not v1)
...
| Color | Meaning |
|---|---|
| Purple | System identity (whyline badge, section labels) |
...
**System messages:** Purple `whyline` badge prefix, clear visual separation from user input.
...
$ whyline
  ...
  ✓ Ready. Your decisions will be stored in ~/.whyline/
...
Never lose data. Always tell the user what happened. `whyline rebuild` is the universal recovery.

### LLM API fails mid-conversation

Retry once silently. If fails again, save conversation state to temp file. Tell user: "Connection lost. Your progress is saved — run `whyline resume` to pick up where you left off."

### LLM returns malformed output

Retry once with same input. If still bad:
- Extraction (no valid `record_complete` record): retry wrap-up prompt once; if still bad, abort session with saved state for `whyline resume`.
- Claim extraction: store record without claims, flag for re-extraction on `whyline rebuild`.
...
Markdown file is always written first. If graph or vector update fails after: log error, tell user "Record saved but indexing incomplete — run `whyline rebuild` to fix."

### Generic handler

For everything else (embedding model corruption, disk full, entity resolution edge cases): clear error message with what happened and what user can do. `whyline rebuild` fixes most things. No silent failures, no stack traces.
...
- Context node merge operation (`whyline merge`) — not v1, schema should support it.
