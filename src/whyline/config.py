...
from whyline.paths import DEFAULT_DATA_DIR, DataPaths, _expand
...
def default_config(data_dir: Path | None = None) -> WhylineConfig:
    root = _expand(data_dir if data_dir is not None else DEFAULT_DATA_DIR)
    return WhylineConfig(
        llm=LlmConfig(),
        embedding=EmbeddingConfig(),
        extraction=ExtractionConfig(),
        data_dir=root,
...
def load_config(paths: DataPaths) -> WhylineConfig:
...
def save_config(paths: DataPaths, config: WhylineConfig) -> None:
...
def _config_from_dict(raw: dict[str, Any], fallback_data_dir: Path) -> WhylineConfig:
...
def _config_to_dict(config: WhylineConfig) -> dict[str, Any]:
    data = {
